...
editor_icon =tk.PhotoImage(file="icons2/editor-icon.png")
main_application.iconphoto(False,editor_icon)
...
###########aline_left....
aline_left_icon=tk.PhotoImage(file="icons2/aline_left.png")
aline_left_btn=ttk.Button(tool_bar,image=aline_left_icon)
aline_left_btn.grid(row=0,column=6,padx=5)

###########aline_center....
aline_center_icon=tk.PhotoImage(file="icons2/aline_center.png")
aline_center_btn=ttk.Button(tool_bar,image=aline_center_icon)
aline_center_btn.grid(row=0,column=7)

###########aline_right....
aline_right_icon=tk.PhotoImage(file="icons2/aline_right.png")
...
def save_file(event=None):
    ...
    try:
        if url :
            ...
            url = filedialog.asksaveasfile(mode="a", defaultextension=".txt",filetypes=(("text file","*.txt"),("all files","*.*")))
...
def exit_file(event=None):
    ...
    try:
        if text_change:
            message_box=messagebox.askyesnocancel('Warnning',"Do you want to save the changes ?")
...
###########edit_commands....
edit.add_command(label="Copy",image=copy_icon,compound=tk.LEFT,accelerator="ctrl+S",command=lambda:text_editor.event_generate("<Control c>"))
...
main_application.bind("<Alt-F4>",exit_file)
